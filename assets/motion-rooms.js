 ...
 * - Cat room toy: a patch of sun follows the pointer with a soft lag and
 *   the featured piece turns towards it (reference: Lusion's light as
 *   material). Touch moves the sun to the tap.
...
export default function roomsMotion({ gsap, ScrollTrigger, lenis }) {
  document.querySelectorAll('[data-room]').forEach((room) => {
    ...
    if (room.dataset.room === 'cat') sunToy(room, gsap);
    if (room.dataset.room === 'dog') dragToy(room, track, strip, trigger, gsap, lenis);
  });
}

function sunToy(room, gsap) {
  const sun = room.querySelector('[data-room-sun]');
  const featured = room.querySelector('.room__featured .product-media__frame');
  if (!sun) return;
  const moveX = gsap.quickTo(sun, 'x', { duration: 1.4, ease: 'power3.out' });
  const moveY = gsap.quickTo(sun, 'y', { duration: 1.4, ease: 'power3.out' });
  ...
  const follow = (event) => {
    ...
    moveX(x);
    moveY(y);
    sun.classList.add('is-lit');
    if (!featured) return;
    const f = featured.getBoundingClientRect();
    const dx = gsap.utils.clamp(-1, 1, (event.clientX - (f.left + f.width / 2)) / box.width);
    const dy = gsap.utils.clamp(-1, 1, (event.clientY - (f.top + f.height / 2)) / box.height);
    turnY(dx * 10);
    turnX(-dy * 6);
  };
  const rest = () => {
    sun.classList.remove('is-lit');
    turnY?.(0);
    turnX?.(0);
  };
